...
1. **抓取文章**: 使用 readability 提取正文，过滤广告和导航元素
2. **提取元数据**: 从HTML meta标签提取作者、日期、描述等信息
3. **翻译内容**: DeepSeek API 翻译，使用占位符保护代码块和链接
4. **下载图片**: 并发下载到 Obsidian 附件文件夹（默认5个并发）
5. **格式转换**: 将图片引用转换为 `![[Attachments/img_xxx.png]]` 格式
6. **构建笔记**: 生成frontmatter元数据和完整的Markdown内容
7. **保存文件**: 写入到Obsidian vault指定目录
8. **输出结果**: 返回包含所有信息的JSON结果
